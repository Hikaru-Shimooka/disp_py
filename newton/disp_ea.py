    ...
    if len(ans_tmp) != 0:
        for w in ans_tmp:
            ...
            for it2 in range(int(1e6)):
                # print('it2', it2)
                z_ec = (omega)/(np.sqrt(2)*k*vec)
                z_eh = (omega)/(np.sqrt(2)*k*veh)
                z_i = (omega)/(np.sqrt(2)*k*vi)
                zeta_ec = plasma_dispersion_func_deriv(z_ec)
                zeta_eh = plasma_dispersion_func_deriv(z_eh)
                zeta_i = plasma_dispersion_func_deriv(z_i)
                zeta_ec2 = plasma_dispersion_func_deriv2(z_ec)
                zeta_eh2 = plasma_dispersion_func_deriv2(z_eh)
                zeta_i2 = plasma_dispersion_func_deriv2(z_i)
                disp = (2*(k**2)
                        - (kec**2)*zeta_ec - (keh**2)*zeta_eh - (ki**2)*zeta_i)
                ...
                # print('it2', it2, dr, di)
                disp_w = (
                    - ((kec**2)/(2*(k**2)))*(1/(np.sqrt(2)*k*vec))*zeta_ec2
                    - ((keh**2)/(2*(k**2)))*(1/(np.sqrt(2)*k*veh))*zeta_eh2
...
if __name__ == '__main__':
    ...
    np.savetxt(
               '../data_newton/ea_teh_eq_{}tec_kwrwi.txt'.format(int(Teh/Tec)),
               ans)
    ...
    plt.savefig('../fig_newton/ea_disp_teh_eq_{}tec.png'.format(int(Teh/Tec)),
                bbox_inches="tight")
    plt.close()
